...
def get_im10k(IMDIR):
    ...
    im_names = sorted(list(dict.fromkeys(list((bname(dname(impath))+'_'+\
                                        bname(dname(dname(impath))),
                                        len(os.listdir(dname(impath))),
                                        dname(impath))
                                        for impath in impaths))))
    im10k = pd.DataFrame(im_names, columns=['im_name', 'n_pic', 'fpath'])
    im10k.index = im10k.im_name
    im10k['tags'] = [splitall(folderpath[1].split(IMDIR,
                                                  maxsplit=1)[1])[1:-1]
                    for folderpath in im10k['fpath'].items()]
    all_tags = sorted(list(dict.fromkeys(flatten([tags[1] 
                                          for tags in \
                                          im10k['tags'].items()]))))
    for row in im10k.iterrows():
        if 'object' in im10k.loc[row[1]].tags:
            im10k.loc[row[1]].im_name == bname(im10k.loc[row[1]].fpath)
            
    matrix = np.array([[bool(tags[1].__contains__(tag))
                      for tag in all_tags]
                      for tags in im10k['tags'].items()])
    imdf = pd.DataFrame(matrix, index=[im_name[0] for im_name in im_names],
                        columns=all_tags)
    im10k['vec'] = [row[1] for row in imdf.iterrows()]
    def categories(dataframe, kword):
        return pd.DataFrame([im10k[row[0]]
                            for row in im10k.iterrows()
                            if kword in im10k['datas'][row[0]].tags])
    return pd.DataFrame(zip(all_tags, 
...
#def categories(dataframe, kword):
#    return pd.DataFrame([image10k.iloc[row[0]]
#                        for row in image10k.iterrows()
#                        if kword in image10k.iloc[row[0]].tags])
#def matrix(kwords=[]):
#    for kword in kwords:
#        categories
#big_d = categories(image10k, 'human')
