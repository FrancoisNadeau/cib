...
def get_im10k(IMDIR):
    ...
    def categories(dataframe, kword):
        return pd.DataFrame([im10k[row[1]]
                            for row in im10k.iterrows()
                            if kword in im10k.iloc[row[0]].tags])
    return pd.DataFrame(zip(all_tags, 
                             [categories(im10k, tag) for tag in all_tags]))

image10k_large = get_im10k(IMDIR)
image10k_large.to_csv('../docs/image10k_concepts.csv')
