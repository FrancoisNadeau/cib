...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 05:59:59 2020
...
def get_im10k(IMDIR):
    '''Generates Dataframe (rows=image concepts, cols=categories).
       Image concepts = Name of image folder
       ...
       For each image concept, membership in a category is reoresented by 1 or
    ...
    conceptnames = list(dict.fromkeys(list((bname(dname(impath)),
                                            len(os.listdir(dname(impath))),
                                            dname(impath)) 
                                           for impath in impaths)))
    cnames = [concept[0] for concept in conceptnames]
    categories = list(dict.fromkeys([label for label in alllabels 
                                     if label not in conceptnames]))
    cnames_sizes = [concept[1] for concept in conceptnames]
    pathparts = [(concept, splitall(concept[2].split(IMDIR, maxsplit=1)[1])[1:-1])
    ...
    imdf = pd.DataFrame(matrix, index=cnames,
                        columns=[categ[0] for categ in categories])
...
def validation(IMDIR):
    ...
    impaths = loadimages(IMDIR)
    # tuple(item name, quantity, directory path)
    # concept names are the image labels per se and don't match WN synsets
    conceptnames = list(dict.fromkeys(list((bname(dname(impath)),
                                            len(os.listdir(dname(impath))),
                                            dname(impath)) 
                                            for impath in impaths)))
    cnames = [concept[0] for concept in conceptnames]
    # Categories are the labels for all concepts >= 1 level higher than 
...
def get_inv():
    '''Converts category membership value (bool) to number (int) of images 
       for each concept. Returns the sum of each column as the total of
       pictures representing each concept.'''
    inv = get_im10k()
    new_matrix = []
...
count = list(count)
concept_count = dict(zip(alllabels, count))
concepts = pd.DataFrame.from_dict(concept_count, orient='index')
